function quadricAnalysis(surf, winSize, verb)
...
for row = 1:size(X_fl, 1)
    ...
    for col=1:size(X_fl, 2)
        ...
        Amat = [1+0*zX(:) 0*zX(:) zX(:) zY(:) 0*zX(:); 0*zX(:) 1+0*zX(:) 0*zX(:) zX(:) zY(:)];
        Bmat = [ fx_fl(:); fy_fl(:)];
        
        param1 = Amat\Bmat;
        J = param1(1:2);
        H = [ param1(3) param1(4); param1(4) param1(5)];
        tmpfx = J(1) + H(1,1)*zX + H(1,2)*zY;
        tmpfy = J(2) + H(2,1)*zX + H(2,2)*zY;
        
        tmpf = J(1)*zX + J(2)*zY + 0.5*( H(1,1)*zX.^2 + (H(1,2)+H(2,1))*zX.*zY + H(2,2)*zY.^2);
...
figure
subplot 121
imagesc([f_depth f_stack]); title(['Depth Map [Ground Truth | Quadric Approximation at block size: ' sprintf('%d', stp2) ']']);
...
figure
subplot 121
imshow(0.5*[zImg1 zImg2]+0.5); title(['Normal Field [Ground Truth | Quadric Approximation at block size: ' sprintf('%d', stp2) ']']);
axis equal; axis tight
subplot 122
...
figure
imshow([imgO imgQ]);
title(['Rendered Surface [Ground Truth | Quadric Approximation at block size: ' sprintf('%d', stp2) ']']);
set(gcf,'units','normalized','outerposition',[0 0 1 1]); drawnow
